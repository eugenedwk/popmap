 ...
 * Returns HSL as "H S% L%" string (without hsl() wrapper) for CSS variables
 */
...
/**
 * Generates CSS variables for a custom primary color theme
 * Includes primary and primary-foreground (auto-calculated for contrast)
 */
export function generatePrimaryColorVars(hexColor: string): React.CSSProperties {
  const hsl = hexToHSL(hexColor);

  // Parse the lightness to determine foreground color
  const parts = hsl.split(' ');
  const lightness = parseInt(parts[2]);

  // Use white foreground for dark colors, dark for light colors
  const foregroundHSL = lightness > 50 ? '0 0% 10%' : '0 0% 100%';
